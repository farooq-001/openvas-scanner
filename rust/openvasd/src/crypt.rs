...
impl TryFrom<String> for Encrypted {
    ...
    fn try_from(s: String) -> Result<Self, Self::Error> {
        ...
        let nonce = parts
            ...
            .ok_or_else(|| ParseError::MissingNonce)??;
        let data = parts
            .next()
...
mod tests {
    ...
    async fn chacha20_encrypt_decrypt() {
        ...
        let encrypter = ChaCha20Crypt::default();
        let encrypted = encrypter.encrypt(data.clone()).await;
        let decrypted = encrypter.decrypt(encrypted).await;
